...
function getNlsPath(): string {
    return join(__dirname, "..", "package.nls.json");
}
...
/**
 * Load the English localization map from package.nls.json
 * Falls back to empty object if file not found or invalid.
 */
function loadNlsMap(): Record<string, string> {
    try {
        const content = readFileSync(getNlsPath(), "utf-8");
        return JSON.parse(content);
    } catch (err) {
        console.warn(err, "⚠️ package.nls.json not found or invalid, skipping localization.");
        return {};
    }
}
 ...
 * If the text is not exactly a placeholder, returns original.
 */
function localize(text: string, nlsMap: Record<string, string>): string {
    if (!text) return text;
    const match = text.match(/^%(.+)%$/);
    if (match && nlsMap[match[1]]) {
        return nlsMap[match[1]];
    }
    return text;
}

function generateConfigurationDocs(): string {
    const nlsMap = loadNlsMap();
    ...
    for (const [key, value] of properties) {
        ...
        // Localize description if it contains a placeholder
        const rawDescription = value.markdownDescription ?? value.description ?? "";
        const description = localize(rawDescription, nlsMap);
